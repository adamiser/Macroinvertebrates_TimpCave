...
for (i in 1:nrow(df.full)) {
  if (df.full$zone[i] == 'Dark') df.full$zone[i] = 'Deep'
}

for (i in 1:nrow(df.new)) {
  if (df.new$zone[i] == 'Dark') df.new$zone[i] = 'Deep'
}
...
mod = lmer(SpecimenAmountPerDay ~ -1 + SpeciesLatinName*Study + (1|TrapNumber),
             data=df.full)
summary(mod)
...
As mentioned above, Acari were not classified as thoroughly in Study 2; thus, a significant difference in Acari was detected. Now, we reclassify the six Acari subspecies in Study 1 (Ameroseiidae, Damaeidae, Peoppiidae, Acaridae, Rhagidiidae, and Stigmaeidae) as "Acari" to more evenly compare total Acari numbers across the 2 studies.

```{r}
df.full.new = df.full
newSpecies = c('Ameroseiidae', 'Damaeidae', 'Peoppiidae', 'Acaridae', 'Rhagidiidae', 'Stigmaeidae')
for (i in 1:nrow(df.full.new)) {
  if (df.full.new[i,'SpeciesLatinName'] %in% newSpecies) df.full.new[i,'SpeciesLatinName'] = 'Acari'
}
...
df.full$zone = relevel(df.full$zone, ref='Deep')
...
We detect a few differences in species populations between deep and twilight areas:

-   Hypogastruridae, Mycetophilidae, Oncopoduridae, Sciaridae, and Tomoceridae seem to be found more in the twilight than the deep.
-   Many other species (all the ones with a dot) seem to be found more in the deep zones.

Now, this result seems odd, since we actually see every one of these species is found more in the deep than in the twilight. HOWEVER, it's very important to note that there are many more traps in the deep than in the twilight - in fact, in Study 1, 71/83 traps are in the deep, and in Study 2, 63/75 traps are in the deep. This model, through averaging, assumes that because there are more traps in the deep there should be proportionally more animals found in the deep. Thus, despite there being more of these species found in the deep traps altogether, the model suggests there are actually more of these species in the twilight because, proportionally per trap, there were more found in the twilight areas.

To demonstrate: In total across both studies, there were 290 Sciaridae found in the deep and only 135 found in the twilight. However, when you account for the fact that 84% of the traps were in the deep, we find only 2.16 Sciaridae per deep trap and a whopping 5.625 per twilight trap! Our model deems this difference large enough to be significant. Thus, we say there are more Sciaridae in the twilight than in the deep.
...
ggplot(data=mds_result, mapping=aes(x=X,y=Y,color=Zone)) +
  geom_point(size=3) +
  scale_color_manual(values = c("Deep" = 'black', "Twilight" = 'gray')) +
  theme(axis.text.y = element_text(colour = "black", size = 14, face = "bold"), 
        axis.text.x = element_text(colour = "black", size=14, face = "bold"), 
        legend.text = element_text(size = 24, face ="bold", colour ="black"), 
        legend.position = "right", 
...
We don't see much in this plot. It seems that deep and twilight are fairly similar.
...
The ANOSIM actually tells us that twilight and deep environments are different. Interesting. The animals reported above seem to frequent twilight areas more than deep. It's interesting that some of them are different from the animals our model predicted. It's likely due to the same reason as stated above - even though they were found more in the deep, when we look at "per trap", they were more frequent in the twilight.
...
It seems the Bray-Curtis dissimilarity might take zone more into account than euclidean does - the outliers are all deep!
